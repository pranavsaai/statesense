...
def get_defects(db: Session = Depends(get_db)):
    defects = db.query(Defect).order_by(Defect.created_at.desc()).all()
    return [{
        "id": d.id,
        "title": d.title,
...
def export_report(cycle_id: int, db: Session = Depends(get_db)):
    ...
    tc_list = [{"id": f"TC{tc.id:03d}", "title": tc.title, "category": tc.category or "",
                "priority": tc.priority or "", "status": tc.status,
                "expected_result": tc.expected_result or "",
                "actual_result": tc.actual_result or ""} for tc in test_cases]

    def_list = [{"title": d.title, "severity": d.severity, "status": d.status,
                 "root_cause": d.root_cause or "", "ai_root_cause": d.ai_root_cause or ""} for d in defects]
