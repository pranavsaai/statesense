...
import { Upload, GitCompare, Shield, AlertTriangle, CheckCircle, Zap, FileText, Plus, Loader2 } from 'lucide-react'
...
export default function Compare({ onCycleCreated }) {
  ...
  const FileUploadZone = ({ file, setFile, label, icon }) => (
      ...
      >
        ...
        <motion.button
          ...
          style={{
            ...
            color: loading || !oldFile || !newFile
              ? 'rgba(255,255,255,0.25)'
              : '#ffffff',
            boxShadow: loading || !oldFile || !newFile
              ? 'none'
        ...
        {result && (
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              >
                <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px' }} className="mb-2">
                  Compliance Score
                </div>
                <div className="text-4xl font-bold mb-2"
                  style={{ color: riskColor[result.risk_level] || '#6366f1' }}
                >
              ...
              <div className="ios-card overflow-hidden">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-white">
                    Changes Detected ({result.changes_detected.length})
                  </span>
                </div>
                {result.changes_detected.map((change, i) => (
                  <motion.div
                    ...
                    style={{
                      borderBottom: i < result.changes_detected.length - 1
                        ? '1px solid rgba(255,255,255,0.05)' : 'none'
                    }}
                  >
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-lg shrink-0 ${
                      change.severity === 'HIGH' ? 'badge-high' :
                      change.severity === 'MEDIUM' ? 'badge-medium' : 'badge-low'
                    }`}>
                      {change.severity}
                    </span>
                    <div>
            ...
            {result.missing_clauses?.length > 0 && (
              <div className="ios-card overflow-hidden">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-white">
                    Missing Compliance Clauses ({result.missing_clauses.length})
                  </span>
                </div>
                {result.missing_clauses.map((clause, i) => (
                  <div key={i} className="px-5 py-3 flex items-center justify-between"
                    style={{
                      borderBottom: i < result.missing_clauses.length - 1
                        ? '1px solid rgba(255,255,255,0.05)' : 'none'
                    }}
                  >
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${
                      clause.risk === 'HIGH' ? 'badge-high' : 'badge-medium'
                    }`}>
                      {clause.risk} RISK
                    </span>
                  </div>
              ...
              <div className="ios-card overflow-hidden">
                ...
                {result.generated_test_cases.map((tc, i) => (
                  <div key={i} className="px-5 py-3"
                    style={{
                      borderBottom: i < result.generated_test_cases.length - 1
                        ? '1px solid rgba(255,255,255,0.05)' : 'none'
                    }}
                  >
